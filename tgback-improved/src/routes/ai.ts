...
import { getAIResponse, summarizeChat, translateText } from '../services/groq';
...
router.get('/summarize/:chatId', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (messages.rows.length === 0) return res.json({ summary: 'Чат пуст.' });
...
router.post('/translate', async (req: AuthRequest, res: Response) => {
  try {
    const { text, targetLang = 'русский' } = req.body;
